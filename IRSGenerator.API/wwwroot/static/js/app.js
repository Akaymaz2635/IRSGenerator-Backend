...
function openLoginModal(allowClose = false) {
  ...
  const sicilInput = container.querySelector('#login-sicil');
  ...
  const doLogin = async () => {
    const sicil    = sicilInput.value.trim();
    ...
    submit.textContent = 'Doğrulanıyor…';

    try {
      const { api } = await import('./api.js');
      const res = await api.auth.login(sicil, password);

      if (res.is_admin) {
        session.set(sicil);
        session.setUser({ sicil, name: res.user?.name || sicil, role: 'admin', id: res.user?.id });
        overlay.classList.add('hidden');
      ...
      session.setUser({
        sicil,
        name: res.user?.name || sicil,
        role: res.user?.role || 'inspector',
      });
      overlay.classList.add('hidden');
      overlay.onclick = null;
      renderSessionBadge();
  ...
  [sicilInput, pwdInput].forEach(el => {
...
// Uygulama açılışında her zaman sicil sor
session.clear();
